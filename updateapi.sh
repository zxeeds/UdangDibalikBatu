...
# ==============================================================================
# Script Update API VPN (Sederhana)
# Hanya menimpa file aplikasi dari repository tanpa mengubah dependensi
# ==============================================================================
...
# 4. Unduh dan Timpa File Baru
download_and_overwrite_files() {
    log_update "info" "Mengunduh dan menimpa file-file aplikasi dari repository..."
    
    # --- Unduh file utama aplikasi ---
    log_update "info" "Mengunduh app.py..."
    wget -q -O "$APP_DIR/app.py" "$REPO_URL/app.py"
    
    # --- Unduh file-file modul aplikasi ---
    # Gunakan loop untuk mempermudah
    local subdirs=("config" "utils" "services" "routes")
    for subdir in "${subdirs[@]}"; do
        log_update "info" "Mengunduh file di subdirektori $subdir..."
        # Buat direktori jika tidak ada
        mkdir -p "$APP_DIR/$subdir"
        # Unduh semua file .py dari subdirektori repo dan timpa yang lama
        wget -q -r -np -nH --cut-dirs=3 -A "*.py" -P "$APP_DIR/$subdir/" "$REPO_URL/$subdir/"
    done
    
    # --- Jangan unduh config.json untuk menjaga pengaturan pengguna ---
    log_update "warning" "File config.json tidak diunduh untuk menjaga pengaturan Anda yang ada."
    
    log_update "success" "File-file aplikasi berhasil diperbarui."
}
...
# 7. Kembalikan ke Versi Lama (Rollback)
rollback_update() {
    ...
    # Hentikan layanan jika masih berjalan
    if is_service_running; then
        stop_service
    fi
...
main() {
    ...
    # Jika download gagal, lakukan rollback
    if ! download_and_overwrite_files; then
        rollback_update
        exit 1
    fi
